# 4. The mixnet proxy's TLS verifies against Android's platform verifier
...
whose 2026-08-10 amendment delegates the proxy's verifier to this
repository.

## Context

zingolib ADR 0021 chose a compiled-in Mozilla root bundle for the
mixnet stack's TLS because the proxy then lived in the zingolib
...
PR #1276 moved the proxy into this repository, whose git dependency on
...
The proxy's TLS uses `rustls-platform-verifier` unpatched, verifying
against Android's own certificate verifier. Three parts wire it:

- `rust/mixnet-proxy/tls-init`, the one hand-written JNI seam, exports
  ...
  outside the proxy crate because `#[no_mangle]` is rejected by the
  proxy's `forbid(unsafe_code)`; the proxy links it on Android targets so
  the symbol ships inside `libmixnet_proxy.so`.
- `NymTlsInit` (Kotlin) loads the proxy library and calls that export.
...
- The app's gradle build consumes the verifier's Kotlin component from
  ...
  halves always share the one version the proxy's lockfile pins.
...
initialization. The invariant to preserve is order — no proxy TLS
