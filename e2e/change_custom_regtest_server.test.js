...
describe('Change the Server.', () => {
  ...
  it('Go settings & change to a correct Regtest server URI.', async () => {
    //await sleep(4000);
    ...
    await waitFor(element(by.id('settings.custom-server')))
      .toBeVisible()
      .withTimeout(sync_timeout);
    await element(by.id('settings.custom-server')).tap();

    // waiting for the custom server field
    await waitFor(element(by.id('settings.custom-server-field')))
      .toBeVisible()
      .whileElement(by.id('settings.scroll-view'))
      .scroll(100, 'down');
    await element(by.id('settings.custom-server-field')).replaceText(
      'http://10.0.2.2:20000',
    );

    // waiting for the toggle, tap on regtest
    await waitFor(element(by.id('settings.custom-server-chain.regtest')))
      .toBeVisible()
      .whileElement(by.id('settings.scroll-view'))
      .scroll(100, 'down');
    ...
    // restart the App with the new regtest serveruri, without any wallet.
