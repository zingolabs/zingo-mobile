...
describe('Maintains correct information while tx pending', () => {
  ...
  it('should send from orchard to sapling pool', async () => {
    ...
    await waitFor(element(by.id('orchard-total-balance.big-part')))
      .toBeVisible()
      .withTimeout(sync_timeout);
    await expect(element(by.id('orchard-total-balance.big-part'))).toHaveText(
      ' 0.0096',
    );
    await expect(
      element(by.id('orchard-total-balance.small-part')),
    ).not.toBeVisible();
    await expect(
      element(by.id('orchard-confirmed-balance.big-part')),
    ).toHaveText(' 0.0000');
    await expect(
      element(by.id('orchard-confirmed-balance.small-part')),
    ).not.toBeVisible();
    await expect(element(by.id('sapling-total-balance.big-part'))).toHaveText(
      ' 0.0002',
    );
    await expect(
      element(by.id('sapling-total-balance.small-part')),
    ).not.toBeVisible();
    await expect(
      element(by.id('sapling-confirmed-balance.big-part')),
    ).toHaveText(' 0.0000');
    await expect(
      element(by.id('sapling-confirmed-balance.small-part')),
    ).not.toBeVisible();
    await expect(element(by.id('transparent-balance.big-part'))).toHaveText(
      ' 0.0000',
    );
    await expect(
      element(by.id('transparent-total-balance.small-part')),
    ).not.toBeVisible();
