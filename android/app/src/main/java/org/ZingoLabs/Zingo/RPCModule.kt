...
class RPCModule internal constructor(private val reactContext: ReactApplicationContext?) : ReactContextBaseJavaModule(reactContext) {
    ...
    @ReactMethod
    fun getAddressesInfo(receivers: String, promise: Promise) {
        CoroutineScope(Dispatchers.IO).launch {
            try {
                uniffi.zingo.initLogging()
                val resp = uniffi.zingo.getAddresses(receivers)

                withContext(Dispatchers.Main) {
                    promise.resolve(resp)
                }
            } catch (e: Exception) {
                val errorMessage = "Error: addresses: ${e.localizedMessage}"
                Log.e("MAIN", errorMessage, e)
    ...
    @ReactMethod
    fun zecPriceApiKeyProcess(key: String, promise: Promise) {
        CoroutineScope(Dispatchers.IO).launch {
            try {
                uniffi.zingo.initLogging()
                val resp = uniffi.zingo.zecPriceApiKey(key)

                withContext(Dispatchers.Main) {
                    promise.resolve(resp)
                }
            } catch (e: Exception) {
                val errorMessage = "Error: zec price API key: ${e.localizedMessage}"
                Log.e("MAIN", errorMessage, e)
