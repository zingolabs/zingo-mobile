...
class BackgroundSyncWorker(context: Context, workerParams: WorkerParameters) : Worker(context, workerParams) {
    ...
    override fun doWork(): Result {
        ...
        if (exists) {
            ...
            val balance = uniffi.rustlib.execute("balance", "")
            ...
            val noInterrupting = uniffi.rustlib.execute("interrupt_sync_after_batch", "false")
            ...
            val syncing = uniffi.rustlib.execute("sync", "")
    ...
    private fun stopSyncingProcess() {
        var status = uniffi.rustlib.execute("syncstatus", "")
        ...
        while (inProgress) {
            // interrupt
            val interrupting = uniffi.rustlib.execute("interrupt_sync_after_batch", "true")
            Log.i("SCHEDULED_TASK_RUN", "Interrupting sync: $interrupting")

            // blocking the thread for 0.5 seconds.
            Thread.sleep(500)

            status = uniffi.rustlib.execute("syncstatus", "")
            Log.i("SCHEDULED_TASK_RUN", "status response $status")
